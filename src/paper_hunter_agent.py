...
class PaperHunterAgent:
    ...
    def __init__(self, user_keywords: List[str]):
        ...
        Args:
            user_keywords: List of keywords to filter papers
        """
    ...
    def search_semantic_scholar(self, arxiv_papers: List[Dict[str, Any]], days_back: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        for arxiv_paper in arxiv_papers:
            try:
                ...
                response = requests.get(url, params={"fields": "title,authors,year,url,abstract,citationCount"})
                ...
                if response.status_code == 200:
                    ...
                    for citation in data.get("data", []):
                        ...
                        if not self._matches_keywords_text(paper.get("title", "") + " " + paper.get("abstract", "")):
                            continue
                ...
                self.logger.error(f"Error searching Semantic Scholar for {arxiv_paper['arxiv_id']}: {e}")
                
        return additional_papers
    
    def filter_and_deduplicate(self, papers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def hunt_papers(self, max_papers: int = 10) -> str:
        ...
        if len(filtered_papers) < 3:
            ...
            semantic_papers_extended = self.search_semantic_scholar(arxiv_papers_extended, days_back=17)
